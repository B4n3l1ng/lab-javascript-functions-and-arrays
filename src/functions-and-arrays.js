...
function findLongestWord(array) {
  ...
  if (array.length === 0 ){
    ...
    return array[0];
      ...
      if (array[index].length > longestWord) {
        result = array[index];
        longestWord = array[index].length;
      } else {
        continue;
      }
    }
...
function uniquifyArray(array) {
  ...
  if (array.length === 0) {
    ...
    for (let i =0; i < array.length; i++) {
      const arrWord = array[i];
      if (newArray.indexOf(arrWord) === -1) {
        newArray.push(arrWord);
      } else {
        continue;
      }
    }
...
function greatestProductOfDiagonals(array) {
  ...
  for (let i=3; i <= rows-2; i++) {
    for (let j=3; j <= columns-2 ; j+=1) {
      product1 = array[i-3][j-3] * array[i-2][j-2] * array[i-1][j-1] * array[i][j];
      product2 = array[i+3][j+3] * array[i+2][j+2] * array[i+1][j+1] * array[i][j];
      product3 = array[i-3][j+3] * array[i-2][j+2] * array[i-1][j+1] * array[i][j];
      product4 = array[i+3][j-3] * array[i+2][j-2] * array[i+1][j-1] * array[i][j]
      if (product1 >= product2) {
        if (product1 >= product3) {
          if (product1 >= product4) {
            productFinal=product1;
          }else{
            productFinal=product4;
          }
        } else if (product3>product4) {
          productFinal = product3;
        } else {
          finalProduct=product4;
        }
